...
def test_model(args):
    ...
    # todo 1 template = torch.zeros((1, 56))  # 没有动作作为template FLAME->1*5023*3
    ...
    out_file_name = test_name + "_condition_" + args.condition  # mesh 文件名
...
def render(args):
    ...
    test_name = os.path.basename(wav_path).split(".")[0]
    # out_file_name = test_name + "_" + args.dataset + "_" + args.subject + "_condition_" + args.condition
    out_file_name = test_name + "_condition_" + args.condition
    predicted_vertices_path = os.path.join(args.result_path, out_file_name + ".npy") #定点运动预测结果路径生成
    # TODO
    # template_file = f"data/{args.dataset}/templates/face_template.obj"
    # template_file = f"data/{args.dataset}/templates/F2.obj" # xiugai
    dataset_dir = "data/" + args.dataset
    if args.dataset=="vocaset":
        template_file = os.path.join(dataset_dir, "FLAME_sample.ply")
    else:
        template_file = f"data/{args.dataset}/templates/F2.obj"
...
def main():
    ...
    parser.add_argument("--model_name", type=str, default="face_diffuser_mead21-2-2230-1000_50")
    # parser.add_argument("--model_name", type=str, default="face_diffuser_mead21-3740-transform0-1000f512-l8_50")
    # parser.add_argument("--model_name", type=str, default="face_diffuser_mead21-2-2230-500_50")
    # parser.add_argument("--model_name", type=str, default="face_diffuser_mead21-3740-1000_50")
    ...
    parser.add_argument("--feature_dim", type=int, default=256, help='GRU Vertex Decoder hidden size') # 256 512
    ...
    # parser.add_argument("--wav_path", type=str, default="demo/wav/contempt1622.m4a",
                        # help='path of the input audio signal in .wav format')
    parser.add_argument("--wav_path", type=str, default="demo/wav/pcnl/happy-z-cn.m4a",
                        help='path of the input audio signal in .wav format')
    parser.add_argument("--result_path", type=str, default="demo/result/pcnl", help='path of the predictions in .npy format')
    parser.add_argument("--condition", type=str, default="happy", help='select a conditioning subject from train_subjects') # 控制生成序列的说话风格（运动mesh）
    # parser.add_argument("--condition", type=str, default="happy", help='select a conditioning subject from train_subjects') # 控制生成序列的说话风格（运动mesh）
    parser.add_argument("--subject", type=str, default="M1",
                        help='select a subject from test_subjects or train_subjects') # X 控制生成序列的身份（shape）
    ...
    parser.add_argument("--output_fps", type=int, default=30,
                        help='fps of the visual data, BIWI was captured in 25 fps') # 30 25
    ...
    parser.add_argument("--diff_steps", type=int, default=1000) # 500 1000
    parser.add_argument("--device_idx", type=int, default=0)
    parser.add_argument("--gru_dim", type=int, default=256) # 256 512
