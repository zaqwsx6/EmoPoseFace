...
class GaussianDiffusion:
    ...
    # todo 02 **** add 7loss （args）
    def training_losses_mul1(self, args, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        """
        Compute training losses for a single timestep.
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            verts_gt = torch.cat([target[:, :, :50],target[:, :, -3:]],dim=2)
            verts_pred = torch.cat([model_output[:, :, :50],model_output[:, :, -3:]],dim=2)
            # todo ***
            if args.l_vert>0:
                loss_vert = self.l2_loss(verts_gt, verts_pred)

            if args.l_vel > 0:
                vel_gt = verts_gt[:, 1:] - verts_gt[:, :-1]
                vel_pred = verts_pred[:, 1:] - verts_pred[:, :-1]
                loss_vel = self.l2_loss(vel_gt, vel_pred)

            if args.l_smooth > 0:
                vel_pred = verts_pred[:, 1:] - verts_pred[:, :-1]
                loss_smooth = self.l2_loss(vel_pred[:, 1:], vel_pred[:, :-1])

            # head pose
            # if not args.no_head_pose:
            # if args.rot_repr == 'aa':
            head_pose_gt = target[:, :, 50:53]
            head_pose_pred = model_output[:, :, 50:53]
            # else:
            #     raise ValueError(f'Unknown rotation representation {args.rot_repr}!')

            if args.l_head_angle > 0:
                loss_head_angle = self.l2_loss(head_pose_gt, head_pose_pred)

            if args.l_head_vel > 0:
                head_vel_gt = head_pose_gt[:, 1:] - head_pose_gt[:, :-1]
                head_vel_pred = head_pose_pred[:, 1:] - head_pose_pred[:, :-1]
                loss_head_vel = self.l2_loss(head_vel_gt, head_vel_pred)

            if args.l_head_smooth > 0:
                head_vel_pred = head_pose_pred[:, 1:] - head_pose_pred[:, :-1]
                loss_head_smooth = self.l2_loss(head_vel_pred[:, 1:], head_vel_pred[:, :-1])
        ...
        return terms , terms["loss"], loss_vert, loss_vel, loss_smooth, loss_head_angle, loss_head_vel, loss_head_smooth

    #todo 03 loss 基于facemesh 和 flame 的loss设计
    def training_losses_mesh_flame(self, args, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        """
        Compute training losses for a single timestep.
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            ...
            # verts_gt = torch.cat([target[:, :, :50],target[:, :, -3:]],dim=2)
            # verts_pred = torch.cat([model_output[:, :, :50],model_output[:, :, -3:]],dim=2)
            verts_gt = target[:, :, :15069]
            verts_pred = model_output[:, :, :15069]
            # todo *** all >
            if args.l_vert >= 0:  # 原 > 现改为>=
                loss_vert = self.l2_loss(verts_gt, verts_pred)
            ...
            # head_pose_gt = target[:, :, 50:53]
            # head_pose_pred = model_output[:, :, 50:53]
            head_pose_gt = target[:, :, -3:]
            head_pose_pred = model_output[:, :, -3:]
            # else:
            #     raise ValueError(f'Unknown rotation representation {args.rot_repr}!')
            # todo all >
            if args.l_head_angle >= 0:
        ...
        # return terms , terms["loss"], loss_vert, loss_vel, loss_smooth, loss_head_angle, loss_head_vel, loss_head_smooth
        return terms , loss_vert, loss_vel, loss_smooth, loss_head_angle, loss_head_vel, loss_head_smooth
